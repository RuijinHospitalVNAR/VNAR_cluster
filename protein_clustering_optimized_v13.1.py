    def process_single_file(self, cif_file):
        ...
        try:
            # 提取坐标和残基信息
            ab_coords, ab_residues, ab_info = self.extract_ca_coords_with_residues(cif_file, [self.antibody_chain])
            ag_coords, ag_residues, ag_info = self.extract_ca_coords_with_residues(cif_file, self.antigen_chains)
            ...
            return None, None, Noneimport os
import numpy as np
...
class ProteinClusterAnalyzer:
    ...
    def _setup_amino_acid_properties(self):
        ...
        self.aa_properties = {
            # 疏水性氨基酸
            'ALA': {'hydrophobic': 1, 'hydrophilic': 0, 'positive': 0, 'negative': 0, 'polar': 0, 'aromatic': 0, 'sulfur': 0, 'size': 1},  # 丙氨酸
            'VAL': {'hydrophobic': 1, 'hydrophilic': 0, 'positive': 0, 'negative': 0, 'polar': 0, 'aromatic': 0, 'sulfur': 0, 'size': 2},  # 缬氨酸
            'LEU': {'hydrophobic': 1, 'hydrophilic': 0, 'positive': 0, 'negative': 0, 'polar': 0, 'aromatic': 0, 'sulfur': 0, 'size': 3},  # 亮氨酸
            'ILE': {'hydrophobic': 1, 'hydrophilic': 0, 'positive': 0, 'negative': 0, 'polar': 0, 'aromatic': 0, 'sulfur': 0, 'size': 3},  # 异亮氨酸
            'PRO': {'hydrophobic': 1, 'hydrophilic': 0, 'positive': 0, 'negative': 0, 'polar': 0, 'aromatic': 0, 'sulfur': 0, 'size': 2},  # 脯氨酸
            'MET': {'hydrophobic': 1, 'hydrophilic': 0, 'positive': 0, 'negative': 0, 'polar': 0, 'aromatic': 0, 'sulfur': 1, 'size': 3},  # 蛋氨酸
            'PHE': {'hydrophobic': 1, 'hydrophilic': 0, 'positive': 0, 'negative': 0, 'polar': 0, 'aromatic': 1, 'sulfur': 0, 'size': 4},  # 苯丙氨酸
            'TRP': {'hydrophobic': 1, 'hydrophilic': 0, 'positive': 0, 'negative': 0, 'polar': 0, 'aromatic': 1, 'sulfur': 0, 'size': 5},  # 色氨酸
            
            # 极性不带电氨基酸
            'SER': {'hydrophobic': 0, 'hydrophilic': 1, 'positive': 0, 'negative': 0, 'polar': 1, 'aromatic': 0, 'sulfur': 0, 'size': 1},  # 丝氨酸
            'THR': {'hydrophobic': 0, 'hydrophilic': 1, 'positive': 0, 'negative': 0, 'polar': 1, 'aromatic': 0, 'sulfur': 0, 'size': 2},  # 苏氨酸
            'CYS': {'hydrophobic': 0, 'hydrophilic': 1, 'positive': 0, 'negative': 0, 'polar': 1, 'aromatic': 0, 'sulfur': 1, 'size': 1},  # 半胱氨酸
            'TYR': {'hydrophobic': 0, 'hydrophilic': 1, 'positive': 0, 'negative': 0, 'polar': 1, 'aromatic': 1, 'sulfur': 0, 'size': 4},  # 酪氨酸
            'ASN': {'hydrophobic': 0, 'hydrophilic': 1, 'positive': 0, 'negative': 0, 'polar': 1, 'aromatic': 0, 'sulfur': 0, 'size': 2},  # 天冬酰胺
            'GLN': {'hydrophobic': 0, 'hydrophilic': 1, 'positive': 0, 'negative': 0, 'polar': 1, 'aromatic': 0, 'sulfur': 0, 'size': 3},  # 谷氨酰胺
            
            # 带正电氨基酸
            'LYS': {'hydrophobic': 0, 'hydrophilic': 1, 'positive': 1, 'negative': 0, 'polar': 1, 'aromatic': 0, 'sulfur': 0, 'size': 4},  # 赖氨酸
            'ARG': {'hydrophobic': 0, 'hydrophilic': 1, 'positive': 1, 'negative': 0, 'polar': 1, 'aromatic': 0, 'sulfur': 0, 'size': 4},  # 精氨酸
            'HIS': {'hydrophobic': 0, 'hydrophilic': 1, 'positive': 1, 'negative': 0, 'polar': 1, 'aromatic': 1, 'sulfur': 0, 'size': 3},  # 组氨酸
            
            # 带负电氨基酸
            'ASP': {'hydrophobic': 0, 'hydrophilic': 1, 'positive': 0, 'negative': 1, 'polar': 1, 'aromatic': 0, 'sulfur': 0, 'size': 2},  # 天冬氨酸
            'GLU': {'hydrophobic': 0, 'hydrophilic': 1, 'positive': 0, 'negative': 1, 'polar': 1, 'aromatic': 0, 'sulfur': 0, 'size': 3},  # 谷氨酸
            
            # 特殊氨基酸
            'GLY': {'hydrophobic': 0, 'hydrophilic': 0, 'positive': 0, 'negative': 0, 'polar': 0, 'aromatic': 0, 'sulfur': 0, 'size': 0},  # 甘氨酸（最小）
        }
    ...
    def extract_ca_coords_with_residues(self, cif_file, chain_ids):
        """提取指定链的Cα原子坐标和残基信息"""
        try:
                ...
                for chain in model:
                    if chain.id in chain_ids:
                        ...
                        if chain_coords:
                            ...
                            residue_names.extend(chain_residues)
                            chain_info[chain.id] = len(chain_coords)
            
            return np.array(ca_coords), residue_names, chain_info
        except Exception as e:
    ...
    def compute_contact_map_with_residues(self, coords_ab, coords_ag, residues_ab, residues_ag):
        ...
        contact_ab_residues = [residues_ab[i] for i in contact_rows]
        contact_ag_residues = [residues_ag[i] for i in contact_cols]
    ...
    def _calculate_charge_complementarity(self, ab_residues, ag_residues):
        ...
        # 电荷互补性：正电荷与负电荷的匹配程度
        complementarity = (ab_positive * ag_negative + ab_negative * ag_positive) / max(len(ab_residues) + len(ag_residues), 1)
    ...
    def _calculate_hydrophobic_interaction(self, ab_residues, ag_residues):
        ...
        # 疏水相互作用：双方疏水残基的乘积
        interaction = (ab_hydrophobic * ag_hydrophobic) / max(len(ab_residues) * len(ag_residues), 1)
        return interaction
    ...
    def _calculate_polar_interaction(self, ab_residues, ag_residues):
        ...
        # 极性相互作用
        interaction = (ab_polar * ag_polar) / max(len(ab_residues) * len(ag_residues), 1)
        return interaction
